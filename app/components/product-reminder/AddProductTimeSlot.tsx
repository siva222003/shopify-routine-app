...
export default function AddProductTimeSlot({ form }: Props) {
  ...
  const renderSlotFields = (
    ...
    const mealError = isConsumable ? slot.error(`meal`) : undefined;
        ...
        <BlockStack gap="100">
          {isConsumable && (
            <Select
              ...
              name={`timeSlotsConsumable[${index}].meal`}
              placeholder="Select"
              onChange={(e) => slot.setValue(`meal`, e)}
              value={slot.value(`meal`)}
              error={mealError}
            />
          )}
          ...
          <InlineStack gap="400">
            <RadioButton
              label="Before Meal"
              name={`timeSlots${isConsumable ? "Consumable" : "AppBased"}[${index}].timing`}
              value="beforeMeal"
              checked={slot.value("timing") === "beforeMeal"}
              onChange={() => slot.setValue(`timing`, "beforeMeal")}
            />
            <RadioButton
              label="After Meal"
              name={`timeSlots${isConsumable ? "Consumable" : "AppBased"}[${index}].timing`}
              value="afterMeal"
              checked={slot.value("timing") === "afterMeal"}
              onChange={() => slot.setValue(`timing`, "afterMeal")}
            />
          </InlineStack>
        </BlockStack>
